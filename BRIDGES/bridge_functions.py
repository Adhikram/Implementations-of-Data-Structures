...
def find_bridges_in_radius(bridges: List[list], lat: float, lon: float,
    ...
    l=[]
    for x in bridges:
        if calculate_distance(lat, lon, x[COLUMN_LAT ], x[COLUMN_LON]) <=float( radius):
            if x[COLUMN_ID] not in exclusions:
                l.append(x[COLUMN_ID])
...
def calculate_average_condition(bridge: list, start: int, stop: int) -> float:
    ...
    for x in range(len(bridge[COLUMN_BCI][INDEX_BCI_YEARS])) :
        if bridge[COLUMN_BCI][INDEX_BCI_YEARS][x] >= str(start) and bridge[COLUMN_BCI][INDEX_BCI_YEARS][x]<str(stop):
            temp=float(bridge[COLUMN_BCI][INDEX_BCI_SCORES][x])
            if temp != MISSING_BCI:
                result += temp 
                count+=1
    if count==0:
...
def inspect_bridge(bridges: List[list], bridge_id: int, date: str,
    ...
    for x in range(len( bridges)):
        if bridges[x][COLUMN_ID]==bridge_id:
            bridges[x].insert(COLUMN_LAST_INSPECTED,date)
            bridges[x].remove(bridges[x][COLUMN_LAST_INSPECTED+1])            
            bridges[x][COLUMN_BCI][INDEX_BCI_YEARS].insert(0,date[6:])
            bridges[x][COLUMN_BCI][INDEX_BCI_SCORES].insert(0,bci)
            
    #print(bridges)

def rehabilitate_bridge(bridges, bridge_ids,
    ...
    """    
    for x in range(len(bridges)):
        if bridges[x][COLUMN_ID] in bridge_ids:
            if is_major:
                bridges[x].insert(COLUMN_LAST_MAJOR_REHAB,new_year)
            else:
                bridges[x].insert(COLUMN_LAST_MINOR_REHAB,new_year)
...
def find_worst_bci(bridges: List[list], bridge_ids: List[int]) -> int:
    ...
    # get_bridge_condition(example_bridges, 1)
    for x in bridge_ids:
        y=get_bridge_condition(bridges, x)
        if result > y:
            result=y
            ans=x
        if result==y:
...
def map_route(bridges: List[list], lat: float, lon: float,
    ...
    ex=[]
    while max_bridges!=0 :
        list_of_bridges=find_bridges_in_radius(bridges, lat, lon, radius, ex)
        if len(list_of_bridges)==0:
            break
        lowest_id=find_worst_bci(bridges, list_of_bridges)
        ex.append(lowest_id)
        l.append(lowest_id)
        lowest_id=find_bridge_by_id(bridges,lowest_id)
        lat=lowest_id[COLUMN_LAT]
        lon=lowest_id[COLUMN_LON]
        max_bridges-=1
    return l
...
def clean_span_data(raw_spans: str) -> List[float]:
    ...
    flag=False
    temp=""
    for x in range(6,len(raw_spans)):
        if raw_spans[x-1]=='=':
            flag=True
            temp=""
        if  raw_spans[x]==';':
...
def clean_bci_data(bci_years: List[str], start_year: int, bci_scores: list) -> \
    ...
    for x in bci_scores:
        temp.append(x)
    for _ in range(len(bci_scores)):
        bci_scores.pop()
        
    for x in temp:
        bci_years.append(str(start_year))
        start_year -= 1

        if x=='':
            bci_scores.append(MISSING_BCI)
        else:
            bci_scores.append(float(x))
...
def clean_data(data: List[list], start_year: int) -> None:
    ...
    for row in data:
        ...
        row[COLUMN_DECK_LENGTH]=int(row[COLUMN_DECK_LENGTH])
        bci_years = []
